...
if (username!=null) {
	try{
		...
		ResultSet rs=smt.executeQuery("select * from staff where email='"+username+"'");
		if(rs.next())
		{
		...
		{
			int i = smt.executeUpdate("insert into staff(`name`,`email`,`password`,`rpassword`) values('"+userid+"','"+username+"','"+useremail+"','"+useremail1+"')");
			out.println("<script> alert('You Registered successfully!'); window.location.href='form-login.html';</script>");
		}
